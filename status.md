...
| | |
|---|---|
| **`main` HEAD** | `84e5c19` — **Market Universe (ADR-022) merged, 2026-09-08** (owner-approved 84e5c19 "merge-ready", fast-forwarded `dev1/market-universe` → `main`, `1971d7a..84e5c19`, 12 commits, clean fast-forward — no divergence on `main` since the branch point). Per-market instrument-spec pinning, risk/execution overrides, asset-class-aware trading calendars (fail-closed on an unapproved session policy), per-market risk ledger, Crumblr-owned fail-closed broker-symbol resolution, and — across three owner corrective review rounds — real end-to-end wiring of all of the above plus cross-market capsule-routing and reconciliation-history/order-frequency isolation in `ExecutionOrchestrator`. Full detail: §13 eighty-eighth through ninety-first entries, `review/adr/ADR-022-market-universe.md`, `review/DEVIATIONS.md` D-060. `order_send` stays NO-GO; `config/paper.yaml`'s BTC/USD entry stays `enabled: false` throughout |
