...
| | |
|---|---|
| **`main` HEAD** | `9553a5843309137eed69c514e415e6dbb0b0a5ad` — **CRUMBLR DEMO EXECUTION CHAPTER — ENGINEERING COMPLETE, owner-declared 2026-09-16** (chapter opened on top of the `bb38029` row below). Four slices landed and merged in sequence: (1) **FINAL Risk budget defect found and fixed** — `revalidate_fixed_volume_at_execution_time()` was comparing carried risk against `evaluate()`'s own rounded-down sizing artifact (`fresh.risk_amount`) instead of the true ADR-001 budget (`current_equity * requested_risk_fraction`), evidenced live by real attempt 3/4/6 numbers; merged `6fa6066e50b9abfd6328735b391b6b17fcb4a261`. (2) **First real Pepperstone DEMO order round-trip** — Attempt 7 (BUY, `rf=0.0005`, sl=200pts/tp=400pts, one-shot durable canary permit) reached a real MT5 fill, followed by an explicit, reviewed one-shot close runner (`scripts/close_demo_canary_position.py`, transport-ambiguity readback bug found/fixed in review) closing the same ticket — the first complete real MARKET entry+close cycle this platform has ever executed. (3) **ICT LIMIT DEMO EXECUTION Slice 1** — real Static ICT Agent's genuine `LIMIT` intents made executable end to end (Agent → Gateway → Risk → Policy → Supervisor → Execution → Reconciliation, unchanged): MT5 `TRADE_ACTION_PENDING`/`ORDER_FILLING_RETURN` support, `PendingOrderState.magic`-based provenance (closes D-049), a `SUBMITTED`→`FILLED` resolver that only fires once the exact submitted ticket is confirmed absent from `pending_orders()` (a v1 premature-FILLED bug found and fixed in review), FINAL Risk priced off the LIMIT reference price, and `scripts/cancel_demo_canary_pending_order.py` (exact-ticket cancel, mirrors the close runner's pattern) — three Dev-1 review rounds, merged `9553a5843309137eed69c514e415e6dbb0b0a5ad`; migration `fb1abaa2d137` (`broker_pending_order_snapshots.magic`) applied to `crumblr_soak`. (4) **Real ICT LIMIT runtime watch** — a dedicated preflight-only watcher (no permit, no `order_send`, no `--apply-canary-config`) has run one real Agent/Gateway/Risk/Policy/Supervisor decision cycle per newly-closed real EUR/USD M5 bar continuously since 2026-09-15, against the real external Static ICT Agent (`crumblr-static-agent-host`, artifact hash `81894d6a9c44ddb0433c15f9779fb0a2e25c0e1eccee232e2fd145d72cb498c5`) — every cycle so far a genuine, database-verified `NO_TRADE`; broker book kept flat throughout (self-checked every cycle, aborts via a `STOPPED_MARKER` if ever violated). **Owner declares the chapter engineering-complete 2026-09-16**: the still-outstanding genuine ICT LIMIT trade is reclassified from unfinished Crumblr engineering to an **acceptance-observation item** — Trader signal frequency/setup quality/strategy improvement are now Trader/Trainer concerns unless evidence shows Crumblr supplied incorrect context. **`order_send`/`feedback_2_0_approved` remain `false` platform-wide** (`config/paper.yaml`) — both real order_send events above ran only under a dedicated, explicitly owner-scoped `config/agent_canary_demo.yaml` override (`submission_enabled=true`/`feedback_2_0_approved=true` in that file only), never a platform-wide flip; no LIVE activation implied or added anywhere in this chapter. Full detail: §13 ninety-eighth entry. Prior merge, referenced above: Agent MVP / Neutral Static Agent Integration, `bb38029`, 2026-09-08 |
| _(superseded row, kept for its own history)_ | `bb38029` — **Agent MVP / Neutral Static Agent Integration DONE, merged, 2026-09-08** (owner-confirmed final state; Crumblr `agent/neutral-static-mvp` fast-forwarded `c3f70c3 → b02ee03` → `main`, no squash; companion `crumblr-static-agent-host` repo fast-forwarded `f1e16b7 → dcc3770` → its own `main`, no squash — this row's own SHA is the status.md-only follow-up commit on top of `b02ee03` that records this). Full route proven end to end: Agent → Gateway → Risk → Platform Policy → External Supervisor → PAPER_LITE → `PAPER_FILLED` (`approved_volume=1.41`, Risk-computed, not Agent-supplied); genuine real EUR/USD NO_TRADE proof; deterministic Pivot-2.2 directional proof; dashboard 8-stage proof complete (real Supervisor verdict + real correlation-bound `PAPER_FILLED`); Dev-1 Core review **PASS**, no Core commit needed (zero diff in `migrations/`, `domain/`, `risk/`, `agent_gateway/`, `config/`). Static Agent StrategyArtifact 6.0 hash `81894d6a9c44ddb0433c15f9779fb0a2e25c0e1eccee232e2fd145d72cb498c5`; strategy compute lives in the external Agent, never in Crumblr Core. **Unchanged, explicitly confirmed:** `feedback_2_0_approved=false`, `submission_enabled=false`, `order_send` NO-GO platform-wide, PAPER_LITE stays fully simulated, no real MT5 mutation added, Static Agent `/health` reports `mt5_capability=false`/`execution_capability=false`. Post-merge regression gate re-run fresh against `main` @ `b02ee03`: ruff/format/mypy clean (239 files formatted, 221 mypy-clean), full suite **1693 passed/3 skipped, 0 failed** (589.6s) against real PostgreSQL, single Alembic head (`8801080869a6`) confirmed; Static Agent gate 71/71 (Dev-2-reported). Full detail: §13 ninety-sixth entry. Prior merge: Dashboard Live Ops, `9ebbe6f..398b2e9`, 2026-09-08 — full detail §13 ninety-fifth entry |
...
| **PAPER_LITE** | Merged to `main` 2026-09-03 (`f645e75`, PR #1, `lite/paper-orchestrator`) — a separate, self-contained track (`application/paper_lite*.py`, `persistence/paper_lite.py`, own tests, `review/PAPER_LITE_DEV3_WORKLOG.md`, `config/paper_lite.yaml`). No Dev-3 session was ever active; **the owner handed this track to Dev 2, 2026-09-05** (own worktree `.claude/worktrees/paper-lite-dev3`, branch `lite/phase-a-convergence`, off post-PR-#3 `main`) — a "Phase-A convergence" pass closed PL-001/PL-003 (already-shipped `assess_open_risk()`/weekly trading-window seams covered them once checked directly) and PL-006, retired the duplicate session-calendar/single-position guard and `paper_lite_agent.py` onto Dev 2's neutral-Agent adapter. Full gate green (1225 unit + 256 integration, own `crumblr_test_dev3` db), pushed to `origin/lite/phase-a-convergence`, **not yet merged**. Not Dev 1's track; zero file overlap confirmed with anything Dev-1-owned. Not narrated further here — see its own worklog |
...
| **`order_send`** | **NO-GO platform-wide, unchanged** — `config/paper.yaml` carries no `feedback_2_0_approved`/`submission_enabled` override, so both default `false`. Two real, explicitly owner-authorized exceptions have executed to date, both under a dedicated `config/agent_canary_demo.yaml` (`submission_enabled=true`/`feedback_2_0_approved=true` in that file only, never the platform default): a full real MARKET entry+close round-trip (2026-09-1x, §13 ninety-eighth entry) and, pending, the eventual first real ICT LIMIT fill (not yet reached — the watcher is still preflight-only, no permit issued as of this entry) |
| **Next formal review target** | `feedback.2.0.md` (routine, per review 1.25 §9's three triggers) — unchanged by the 2026-09-16 chapter-closure declaration, which is an owner engineering-scope call, not a `feedback.2.0` GO. `feedback.1.26.md`/`feedback.1.27.md`/`feedback.1.28.md` (all 2026-09-01) were deliberate owner/reviewer checkpoints (1.28 an explicit early-escalation exception per 1.27 §12's own trigger), not a change to that default |
...
| M5 Paper execution | MT5-INTEGRATED (gated, one-shot) | **NO-GO for continuous/autonomous operation; two real one-shot round-trips proven under an explicit owner-scoped canary override** | Massively advanced since the original twelve-prerequisite list (`review/feedback.1.0.md` §6) — all of Phase 4 (execution eligibility/FINAL Risk/submission gate/ambiguous-recovery/idempotence/flatten/reconciliation) is formally PASSED (review 1.24) and real-terminal validated (§13 forty-fifth, ninety-eighth entries). Two real Pepperstone DEMO order events have actually happened: a full MARKET entry+close round-trip (Attempt 7 + explicit close runner) and real ICT `LIMIT` submission/cancellation support (Slice 1) — both only under a dedicated, explicitly owner-authorized `config/agent_canary_demo.yaml` override (one-shot durable permit, never the platform default). `config/paper.yaml`'s `feedback_2_0_approved`/`submission_enabled` remain `false` — this milestone does not become a general GO until `feedback.2.0.md` says so; see §13 ninety-eighth entry for the full chapter record |
...
| 2026-09-16 | **CRUMBLR DEMO EXECUTION CHAPTER declared engineering-complete**; the outstanding genuine ICT LIMIT trade is reclassified from unfinished Crumblr engineering to an acceptance-observation item | Owner review of the full chapter's evidence: real MARKET entry+close round-trip, real Static Agent → Gateway → Risk → Policy → Supervisor path, real LIMIT execution support (submission/magic-tracking/resolver/cancel), FINAL Risk for a fixed LIMIT price, the required DB migration applied, and a real ICT watcher exercising fresh M5 decisions with zero broker writes — all proven. Trader signal frequency/setup quality/strategy improvement move to the Trader/Trainer track unless evidence shows Crumblr itself supplied incorrect context. Dev 2 continues the watcher (no permit/submission until a genuine accepted LIMIT appears; no Crumblr changes merely because the Trader keeps returning `NO_TRADE`); Dev 1 owes no further review on this chapter absent a concrete Core/execution defect, and the eventual genuine LIMIT proof needs no additional implementation review since it reuses already-reviewed paths unchanged | Continuing to treat sparse/absent genuine ICT signals as an open Crumblr engineering item — rejected, since every layer between the Trader's decision and the broker is now proven, and the remaining variable is the Trader's own setup frequency, not Crumblr's execution chain | Project owner |
...
## Update 2026-09-04 (seventy-ninth entry) — Phase B slice 6: real per-ticket close/flatten (B5) — Phase B complete

```text
Component: mt5_gateway/execution.py, mt5_gateway/demo_execution.py, application/execution_outcome.py, application/execution.py, risk/flatten_gate.py, domain/enums.py, domain/models.py
Milestone: DEMO canary work order, Phase B, item B5 — the last unshipped Phase-B slice
Status before: B1/B2/B3/B4/B7/B8 shipped; B5 not started; B6 explicitly deferred
Status after:  Phase B complete (B6 stays deferred by the work order until continuous-DEMO promotion) — Phase C/AG-012 is the only remaining gate before the real submission chain (entries) can be wired together
```

**The one design revision worth recording: the real close does not happen
inline with the commit.** The first draft had `_commit_flatten()` attempt
the real close immediately after appending `FLATTEN_SUBMISSION_STARTED`,
in the same call. That broke every existing
`test_execution_flatten.py` assertion that a fully-approved config's
*first* `flatten_once()` pass stops at `FLATTEN_SUBMISSION_STARTED`
(`fake.close_all_positions_calls == 0` / `fake.order_send_calls == 0`
immediately after) — and, more importantly, put a real broker write in
the same call stack as the gate decision that had just authorized it.
Reverted: `_commit_flatten()` is **unmodified**. The real close is
attempted only through `_resolve_flatten_outcome()`, reached on the
*next* pass — the same two-step "commit now, act next pass" shape item
6's ambiguous-recovery already established for entries. Every existing
flatten integration test (15, was 11) now passes **unchanged**.

**Completed**

- `mt5_gateway/execution.py::build_close_order_request()` (new) — the
  MT5 request dict for closing one exact ticket, mirroring
  `build_market_order_request()` but always setting `"position":
  instruction.ticket` — the field that tells a hedging-account broker
  which specific position to act on, never symbol/side alone.
- `mt5_gateway/demo_execution.py::DemoOrderSendMt5Gateway.close_position()`
  (new, real) — one per-ticket close, same demo-only guard as
  `order_send`. `.close_all_positions()` (real, was an unconditional
  raise) — reads `positions()` fresh, closes each independently (one
  ticket's failure never blocks the rest), returns the tickets that
  actually closed. This one change makes
  `risk/operator_controls.py::OperatorControls.flatten_positions()` real
  end to end with **zero edits to `operator_controls.py` itself** — it
  already called `broker.close_all_positions(reason=...)`.
  `_decode_order_send_result()` factored out of the old `order_send`
  body so both real mutating calls share one decode, not two.
- `application/execution_outcome.py::close_result_fully_closed()` (new,
  pure) — `OrderState.FILLED` only; `PARTIALLY_FILLED` is deliberately
  **not** treated as closed (a named scope limit, not a silent gap — see
  ADR-020 §8).
- `application/execution.py`: new `FlattenCloseSink` Protocol (narrow —
  one method, `close_position`) and an optional
  `flatten_close_adapter: FlattenCloseSink | None = None` constructor
  param, `None` in every existing caller/test — the exact pre-B5
  behaviour is the default, not a flag someone has to remember to set.
  `_attempt_and_resolve_flatten()` (new, replaces the old pure-observe
  body of `_resolve_flatten_outcome()`, which now only reconstructs
  typed `FlattenInstruction`s from the commitment payload and delegates)
  — attempts a real close only when a real adapter is present *and*
  `flatten_submission_enabled` reads `True` right now (re-checked fresh,
  never trusted from commit time), always re-observes fresh broker state
  before deciding, and — because `FlattenEventType` events are
  append-once per `(flatten_request_id, event_type)`
  (`persistence/flatten.py`) — appends `FLATTEN_OUTCOME_RESOLVED` only
  once a terminal outcome is actually known. A residual after a genuine
  attempt trips `ReasonCode.FLATTEN_CLOSE_FAILED`
  (`_trip_flatten_close_failed`, the same idempotent-trip shape as the
  three existing `_trip_*` helpers) and appends **nothing** — the next
  pass retries automatically, no artificial counter, never a blind
  resubmission of an already-closed ticket.
- `flatten_once()`'s two call sites now trip `OVERNIGHT_EXPOSURE` from
  their own fresh-before-any-attempt `positions` read, **before**
  resolving/committing rather than after — a real close inside that call
  can now change what's actually open, and the old ordering would have
  re-tripped from a stale snapshot moments after a clean resolution.
  `KillSwitch.trip()`'s idempotence means this is a no-op reordering for
  every pre-B5 case.
- `risk/flatten_gate.py::_TOLERATED_HALT_REASONS` gains
  `FLATTEN_CLOSE_FAILED` — the halt a failed close causes must not brick
  the mechanism that would otherwise retry it, same reasoning as the
  four reasons already there.
- New `ReasonCode.FLATTEN_CLOSE_FAILED` (`domain/enums.py`, additive).
  `ExecutionResult.intent_id` widened `UUID` → `UUID | None`
  (`domain/models.py`) — a close has no `TradeIntent`; confirmed via
  grep before widening that nothing anywhere reads this field back out.
  Both logged in `review/INTEGRATION_NOTICES.md` (shared-contract
  territory).
- `review/adr/ADR-020-real-flatten-close.md` written — full reasoning,
  including the reverted same-pass design and why (§5), and an explicit
  observed-but-correct effect: in practice `OVERNIGHT_EXPOSURE` almost
  always wins the kill switch's global `active_reasons` slot before
  `FLATTEN_CLOSE_FAILED` can (§6) — the specific reason stays visible on
  the durable outcome/event regardless, which is where a flatten-specific
  reader should look.
- `review/DEVIATIONS.md`: D-050 narrows to just the still-open
  pre-deadline connectivity watch and the untouched ADR-004 §7 owner
  questions (two of three original pieces closed); D-033 updated to
  reflect the close mechanism now being real though still fully inert;
  D-054 gap 1 gets an accuracy note (a real mechanism exists in code but
  is not yet live-wired, so the BLOCK-not-HALT rationale is unchanged in
  practice). `review/adr/ADR-009-automatic-flatten-submission.md` §3's
  scope table and "what this does not do" prose updated to point at
  ADR-020 rather than restate the now-stale "unbuilt and unreachable"
  claim.

**Evidence**

- New/extended tests: `tests/unit/test_demo_order_send_gateway.py` — 9
  new (`close_position` request shape/decode/demo-guard, `close_all_positions`
  multi-ticket/partial-failure/transport-failure/empty-book), 1 test
  replaced (`test_close_all_positions_still_refuses` → real behaviour),
  `TestNotWiredIntoTheOrchestrator`'s docstring updated, mechanical
  assertion unchanged and still passes. `tests/integration
  /test_execution_flatten.py` — 4 new (`TestRealFlattenClose`: a
  successful close resolves `flattened=True` next pass and stays
  idempotent on a third; a rejected close trips the halt, does not
  brick the gate, and succeeds on retry; a transport failure on one
  ticket does not block a second; an already-externally-closed ticket
  is never re-attempted), all 11 pre-existing tests in the file
  unchanged and still green. `tests/unit/test_flatten_gate.py` — 1 new
  (`FLATTEN_CLOSE_FAILED` joins the tolerated set).
- quality gate: `ruff check .` / `ruff format --check .` / `mypy` all
  clean (192 source files) after every edit in this slice.
- Full suite (unit + integration, real PostgreSQL, `crumblr_test_dev1`):
  **1431 passed / 3 pre-existing skips / 0 failed, 330.20s** — up from
  the seventy-eighth entry's 1418 baseline, net +13 collected after this
  slice's new tests (9 in `test_demo_order_send_gateway.py`: 5
  `TestClosePosition` + 4 `TestCloseAllPositions`; 4 new
  `TestRealFlattenClose` in `test_execution_flatten.py`; 1 new in
  `test_flatten_gate.py`) and one existing test rewritten in place
  (`test_close_all_positions_still_refuses` → real behaviour, same slot,
  not an addition). An intermediate full run mid-slice (before the
  integration-level `TestRealFlattenClose`/flatten-gate tests existed)
  also came back clean at 1426/3/0 — confirms the shared-contract
  widenings (`ExecutionResult.intent_id`, the new `ReasonCode`) broke
  nothing elsewhere in the tree before this slice's own new coverage was
  even added.
- Determinism: not re-run this slice — no replay-path code touched.

**Problems found**

- The same-pass design (§ above) — caught by the *existing* test suite
  immediately, not discovered later. Exactly what that suite is for.
- `_trip_overnight_exposure` ordering: a genuine, if narrow, correctness
  gap the same-pass revision surfaced (a stale `positions` snapshot
  could have re-tripped a halt a clean close had just resolved) — fixed
  by reordering both call sites, not worked around.

**Risk impact**

- None in any shipped config: `flatten_submission_enabled` stays `false`
  everywhere, and no adapter is constructed by any real caller — the
  same "real, tested, structurally inert" status every Phase-B slice
  before this one shipped with.
- The operator control path (`OperatorControls.flatten_positions()`) is
  now genuinely consequential the moment a real `BrokerPort` is handed
  to it — but `OperatorControls` itself is constructed nowhere in
  `src/`/`scripts/` today (confirmed by grep before this slice began),
  so this is not a live capability change.

**Decision**

- Not yet committed — will ask for explicit per-turn approval before
  committing, per standing session pattern.

Next:
- Ask for commit approval; push after re-confirming `origin/main`
  hasn't moved; notify Dev 2 once pushed (informational — no
  cross-track surface change expected).
- **Phase B is complete.** Phase C/AG-012 (one serialized shared
  execution/Risk authority, joint with Dev 2) is the only remaining gate
  before entries can be wired into a real submission chain at all.

---

## Update 2026-09-04 (eightieth entry) — Phase C: single Risk-authority lock (AG-012, ADR-021), Dev-1 side

```text
Component: risk/session.py, persistence/risk_session.py, application/live_decision.py, application/execution.py, application/bootstrap.py
Milestone: Owner/reviewer coordination order Phase C — "Dev 1 + Dev 2 — close AG-012 at the execution convergence point"
Status before: AG-012 open, interim option-2 mitigation only (decision_path.py re-recovers fresh every call, no shared lock, no fix on the LiveDecisionOrchestrator side)
Status after:  Dev-1 side implemented and tested; Dev 2's decision_path.py side is the only remaining piece before AG-012 closes for real
```

**Cross-session-coordinated with Dev 2 before writing any code, two full

rounds — the process working as designed, not just documentation
theatre.** Round 1: drafted ADR-021 adopting Dev 2's own AG-012 design
analysis (`review/AGENT_STATUS.md` §0n, `pg_advisory_xact_lock`,
symbol-keyed); Dev 2 acked, with one precision correction to a
supporting claim. Round 2: while starting implementation, found a real
gap in the acked design before writing a single line — `held()`'s
required-external-`connection` shape had no natural source in either
orchestrator (neither owns a raw `Engine`). Revised to lock-owns-the-
transaction-and-yields-it-out, sent back to Dev 2 explicitly flagged as
a course correction (not silently changed after ack), re-verified
independently on their side (re-grepped `live_decision.py` themselves)
and re-acked, with a second precision correction (`RunRecorder` does
hold an `Engine` privately — the accurate claim is "no orchestrator
*class* owns one directly," not "nothing in reach of one does"). Both
corrections fixed in the ADR in place before implementation began.

**Completed**

- `risk/session.py`: new `RiskLedgerLock` Protocol (`held(canonical_symbol)
  -> AbstractContextManager[Connection | None]`) and
  `InMemoryRiskLedgerLock` (no-op, pairs `InMemoryRiskSessionStore` —
  both are legitimate production code per that class's own docstring,
  "for tests and for replays that should not outlive their process," not
  merely test doubles). `RiskSessionStore.load_latest()`/`.save()` both
  gain an optional `connection: Connection | None = None` param
  (source-compatible — every existing caller that omits it is
  unaffected).
- `persistence/risk_session.py`: new `PostgresRiskLedgerLock` — one
  `pg_advisory_xact_lock(hashtext('risk-ledger:' || canonical_symbol))`
  per symbol, opens its own transaction and yields the connection out.
  Reuses `persistence/agent_gateway.py::lock_assignment()`'s exact
  primitive, not a new mechanism. `PostgresRiskSessionStore.load_latest()`/
  `.save()` updated to route through the given `connection` when present.
- `application/live_decision.py::LiveDecisionOrchestrator.decide_once()`
  — the real behavioural change. No longer recovers the ledger once
  (first call/trading-day rollover) and caches it across calls; now
  recovers, updates and persists fresh **every cycle**, inside one
  `with self._risk_ledger_lock.held(...) as connection:` block, right
  after `positions`/`open_risk` are computed and before the loss-gate/
  session-boundary checks. `session.recover_session()` already derives
  same-day-vs-new-day semantics from `(recorded.trading_day,
  market_day)` itself, so the old manual day-rollover branch
  (`EquityLedger.start_new_session()` called directly from `decide_once()`)
  is gone — one fewer piece of state to keep in sync by hand. **Closed a
  second, separately-found gap in the same change**: persistence
  previously only ran on a cycle that reached a risk-`PASS` decision, so
  a run of `NO_TRADE` decisions of any length left the durable checkpoint
  stale — independent of the AG-012 race itself, named explicitly in
  `review/adr/ADR-021-single-risk-authority-lock.md` §3 rather than
  folded silently into "the AG-012 fix."
- `application/execution.py::ExecutionOrchestrator._process()`'s own
  FINAL Risk `load_latest()` read joins the same lock too — it was
  already fresh every pass (no caching, review 1.23 F-058), so this was
  never part of the race, but it is the literal closest thing to the
  owner work order's own "final-Risk-to-broker-side-effect critical
  section" wording, and the marginal cost is small (no write-back on
  this call site either — lock-for-read-consistency only, same as
  Dev 2's side).
- `application/bootstrap.py::DurableRuntime` gains `risk_ledger_lock`,
  ...
  every real script (`scripts/live_decision.py`,
  `scripts/run_execution_preflight_evidence.py`) picks it up through
  that one composition root, no manual wiring needed at each call site
  beyond threading the new field through.
- Both orchestrators' constructors gain a new **required**
  `risk_ledger_lock` parameter — not source-compatible, the same kind of
  break PL-006's `recover_session()` signature change was (2026-09-03).
  Every real construction site updated in this same change:
  `DurableRuntime`/`build_durable_runtime`, both scripts,
  `tests/integration/_execution_fixtures.py`, every builder in
  `tests/unit/test_live_decision.py`/`tests/integration/test_live_decision.py`.
- Found and fixed one Dev-2-owned test-file ripple while wiring this:
  `tests/unit/test_agent_decision_path.py::ExplodingSessionStore` (a
  local, in-test-method `InMemoryRiskSessionStore` subclass) needed the
  same `connection` parameter added to satisfy the widened Protocol — a
  one-line, purely mechanical fix, not touching any `agent_gateway/`
  production code. Flagged explicitly in
  `review/INTEGRATION_NOTICES.md`, not silently absorbed.

**Evidence**

- New tests: `tests/unit/test_live_decision.py::TestADR021RiskLedgerPersistsEveryCycle`
  (2 — a `NO_TRADE` cycle now persists and acquires the lock exactly
  once; a second, independent orchestrator instance sharing only the
  durable store observes the first's persisted state, proving the ledger
  is no longer retained in-process across calls).
  `tests/integration/test_risk_ledger_lock.py` (2, real PostgreSQL, real
  threads — a second lock holder never starts before the first releases;
  two different symbols' locks do not block each other, proving the key
  is genuinely per-symbol, mirroring
  `test_agent_gateway_store.py::TestRateLimitIsAtomicUnderRealConcurrency`'s
  own "real concurrent connections, real Postgres lock manager"
  discipline for `lock_assignment()`).
- quality gate: `ruff check .` / `ruff format --check .` / `mypy` all
  clean (193 source files).
- Full existing suite re-run unchanged and green throughout — the
  redesign changes ledger recovery/persistence *cadence*, not any
  externally-observable decision/outcome:
  `tests/integration/test_live_decision.py`,
  `test_execution_orchestrator.py`, `test_execution_flatten.py`,
  `tests/unit/test_risk_session.py` (62 together, isolated run), the
  full non-integration suite (1178 passed/1 pre-existing skip), and the
  full suite including integration, real PostgreSQL against the
  isolated `crumblr_test_dev1` database: **1435 passed / 3 pre-existing
  skips / 0 failed, 380.54s** — exactly 1431 (B5's own final count) + 4
  new (2 unit, 2 integration).

**Problems found**

- The required-`connection` design (round 1's acked shape) had no
  natural caller in either orchestrator — caught by re-reading the
  actual target files before writing code, not discovered mid-implementation.
- The second, separate `NO_TRADE`-cycles-never-persist staleness gap —
  found while redesigning `decide_once()` for the AG-012 fix, closed in
  the same change rather than deferred.
- One transient, non-reproducing failure during this work: a full-suite
  run taken *while* other `pytest` processes were still touching the
  same `crumblr_test_dev1` database concurrently (this session's own
  earlier targeted runs, not yet finished) showed 1 failed + 2 errored
  in `test_market_data_store.py::TestTheReplayRecordsWhatItSaw` —
  nothing this change touches. Re-run in isolation (that file alone,
  then the full suite alone, nothing else hitting the database) came
  back clean both times, confirming environment contention from
  concurrent DB access, not a real regression — the same "random
  `relation already exists`" class of failure `CRUMBLR_DEV1_CORE_EXECUTION_INSTRUCTIONS.md`
  §2 already names and instructs to treat this way. Recorded here rather
  than silently reruns-until-green, per this session's own "report
  failures plainly" standard.

**Risk impact**

- None in any shipped config: `order_send` remains unreachable from
  every pipeline regardless of this fix — AG-012 was never a live safety
  gap (FINAL Risk already re-derives everything from real broker state,
  independent of either pipeline's intent-time ledger view), only a
  Phase-C precondition for agent-driven submission to ever be treated as
  real.
- Real behavioural change to a Phase-4-approved, already-reviewed
  component (`LiveDecisionOrchestrator`) — exactly the kind of change
  this session's own standing practice is to write up as its own ADR
  before merging, not slip in quietly. Done here.
...
Next:
- Confirm the full suite's final result, then ask for commit approval;
  push after re-confirming `origin/main` hasn't moved; notify Dev 2 once
  pushed so they can build `decision_path.py`'s side against the real
  thing.
- Once Dev 2's side lands: `review/AGENT_FEEDBACK.md` AG-012 closes for
  real. Phase C is otherwise Dev-1-complete.
...
## Update 2026-09-04 (eighty-second entry) — AG-012 closed on both sides

Dev 2 landed `decision_path.py`'s side of ADR-021 same day:
`evaluate_agent_trade_intent()` acquires `risk_ledger_lock.held(snapshot.symbol)`
around the existing `load_latest()` read, released before Policy
evaluation runs — the same read-only, lock-for-consistency scope this
ADR's §4 specified. `paper_lite.py`'s two call sites and both scripts'
composition roots threaded through mechanically for the forced signature
change (their file, their fix — not the AG-023 gap itself, which is a
separate, still-open, deliberately-untouched item). 4 new tests on their
side (`TestAG012RiskLedgerLockAcquired` — exact symbol, exact connection
threaded through and not just *some* connection, `NO_TRADE` never
touches the lock, and the lock releases before Policy evaluation, proven
with a non-reentrant fake). Full suite on their side: 1217 unit + 255
integration against real PostgreSQL (including this session's own
`test_risk_ledger_lock.py` concurrency proof, imported by the merge), 0
failed. Pushed to `origin/agent/contracts` at `6ac8cf2` — **not yet
merged to `main`**; that stays the existing owner/Dev-2 merge-timing
decision, unchanged by this entry.

`review/AGENT_FEEDBACK.md` AG-012 is now Closed, with Dev 2's own full
resolution recorded there and cross-linked to `review/adr
/ADR-021-single-risk-authority-lock.md`. **Phase C is complete on the
Dev-1 side and AG-012-resolved on both sides** — the compact header
above reflects this. Nothing queued for Dev 1 right now; idle pending
either an owner/Dev-2 decision to merge `agent/contracts`, AG-023
(PAPER_LITE) being picked up by whoever owns that file, or a new work
order.

---

## Update 2026-09-04 (eighty-third entry) — README.md and HANDOVER.md rewritten, both 8 days stale

The owner asked directly whether everything is documented for a reviewer
or supervisor to look at. Honest answer required checking, not assuming:
the detailed audit trail (every ADR, this document, `review/DEVIATIONS.md`,
`review/INTEGRATION_NOTICES.md`, `review/FEEDBACK.md`) was current, but
`README.md` and `HANDOVER.md` — the two documents a reviewer or a new
session reads *first* — both still described the project as it stood
2026-08-26, before Phase 4 formally passed, before any of Phase B, before
the DEV1/DEV2 track split and the external-agent Gateway existed, and
before Phase C/AG-012 closed. Concretely wrong claims, not just outdated
framing: README's own "what is implemented" table said `order_check`/
`order_send` and automatic flatten were both "Not started" (both are
real, gated closed, not absent); test counts were 877/925 against a real
current count over 1400; `HANDOVER.md`'s own "§0 start here" narrative
still named F-051 as *the* blocking checkpoint a full week after it
closed.

Both rewritten from scratch to reflect current reality — the two-track
structure and workspace-isolation setup, every Phase B/C ADR through
ADR-021, the real-but-structurally-unreachable execution/flatten chain,
F-051's full closure, the changed review cadence (`feedback.2.0.md` is
the next *routine* target, not another numbered review), and the one
known open gap (AG-023). Every file path and test name cited was checked
to actually exist first (`ls`/`grep`), not written from memory of what
the codebase used to look like — the exact discipline this document's
own §0.2 addition now names explicitly for anyone orienting themselves
in a stale checkout.

Decision: documentation-only, no code touched — pushed directly, same
practice as the ADR-021 amendment.

Next: nothing changes about what work is queued (§13 eighty-second
entry's own "next" still applies) — this closes a documentation gap, not
an engineering one.

---

## Update 2026-09-04 (eighty-fourth entry) — Dev-2 milestones recorded: F-066 condition 8 closed, AGENT_FEEDBACK.md staleness corrected

Copied in from Dev 2's own report, per the standing "only copy concise,
merged Agent milestones into canonical `status.md`" instruction — not
this session's own engineering. Two things, separate from the AG-012
work already recorded above:

1. **F-066's own tracker row in `review/AGENT_FEEDBACK.md` had drifted
   stale** — it still listed `AgentMarketContextV1` and structural/opaque
   Gateway reason-code handling as open items weeks after both had
   actually shipped. Found and corrected by Dev 2 while re-reading review
   1.28 §10's real nine-condition closing list directly, not trusting
   the tracker's own summary of itself.
2. **F-066 condition 8** ("a second, differently-vocabularied toy agent
   reaches the same Core path with zero Core code changes" — the
   reviewer's own named regression test that a platform, not a
   single-strategy integration, was actually built) was previously only
   proven at `decision_path.py`'s own function-argument level. Closed
   properly: a new test onboards two fully independent agents (distinct
   identity, assignment, artifact hash, and reason-code vocabulary)
   through the real, unmodified `AgentGateway.submit_trade_proposal()`
   entry point, then through `evaluate_agent_trade_intent()`, both
   reaching identical `PASS`/`APPROVE` — the real Gateway boundary, not a
   shortcut around it.

**F-066 is now down to conditions 2 and 7 only, both dependent on the
external Agent Developer's own fork/runtime work** — nothing left to
build on the Crumblr side of either. Pushed to `origin/agent/contracts`
(`35e02f2`), same not-yet-merged-to-`main` status as the rest of that
branch's accumulated work.

Compact header's Dev-2 row rewritten to match (it had also drifted —
predated the AG-012 closure already recorded in this document).
...
the way a shared-contract change would require.

Next: unchanged from the eighty-second entry.

---

## Update 2026-09-04 (eighty-fifth entry) — AG-023 closed (Dev-2 side, recorded here), AG-024 fixed (this session)

**AG-023 — recorded on Dev 2's behalf**, per the standing "only copy
concise, merged Agent milestones" instruction, not this session's own
engineering. With the owner explicitly authorizing Dev 2 to cross the
file-ownership line (no Dev-3 session ever became available),
`application/paper_lite.py::PaperLiteOrchestrator._recover_risk_session()`/
`_persist_risk_session()` now each acquire `risk_ledger_lock.held(canonical_symbol)`
around their own store call, closing the gap `review/adr
/ADR-021-single-risk-authority-lock.md` §1's amendment had recorded
honestly rather than papered over. Dev 2 verified deadlock-safety first
(the two PAPER_LITE-side holds never overlap with
`evaluate_agent_trade_intent()`'s own internal one, despite all three
contending for the same advisory-lock key) and left one tradeoff
explicitly named, not silently resolved: the two methods stay separately
locked rather than one atomic critical section, because PAPER_LITE's
`SimulatedBroker` can fill synchronously within the same `process()`
call — closing that fully would delay a same-cycle fill's P&L
bookkeeping by one cycle. Full detail: ADR-021 §7 (updated in place this
session from Dev 2's report). Their commit `dda707c`, reported not yet
pushed as of this recording.

**AG-024 — this session's own fix.** While closing AG-023, Dev 2's own
self-review surfaced a second gap and explicitly deferred its design to
Dev 1 ("your call, since it's your design"): neither
`RiskLedgerLock.held(...)`'s acquisition nor `RiskSessionStore.save()`
had any exception handling anywhere in the call chain. Unlike
`load_latest()`, which already degrades to `SessionRecord(unreadable=...)`
on its own, a failure here had no handling anywhere —
`scripts/live_decision.py`'s loop only catches `KeyboardInterrupt` — so
a transient database blip during either would have crashed the whole
process, not merely refused one cycle.

Fixed by wrapping the entire `with self._risk_ledger_lock.held(...) as
connection: ...` block (not the lock primitive itself — a
generator-context-manager-wrapping-`engine.begin()` approach for the
primitive to internally distinguish "acquisition failed" from "the
caller's own code after `yield` failed" proved architecturally messy,
and would also wrongly attribute the caller's own unrelated failures to
"lock unavailable") in a plain `try/except Exception` at each Dev-1-owned
call site, converting to the same fail-closed HALT pattern every other
`..._STATE_UNKNOWN` reason code already uses in this codebase:

- `application/live_decision.py::LiveDecisionOrchestrator.decide_once()`
  — wraps the full recover→update→persist block; trips the kill switch
  and returns a skip outcome on failure, never raises.
- `application/execution.py::ExecutionOrchestrator._process()` — wraps
  the lock-acquisition-plus-`recover_session()` block; appends
  `FINAL_RISK_BLOCKED` via the existing `_refuse()` helper on failure.

New `ReasonCode.RISK_LEDGER_LOCK_UNAVAILABLE` (`domain/enums.py`,
additive, same `..._STATE_UNKNOWN`-shaped family as
`SAFETY_STATE_UNKNOWN`/`DECISION_STATE_UNKNOWN`/`OPEN_RISK_UNKNOWN` but
deliberately its own distinct code — an operator reading the halt reason
should be able to tell the ledger lock/store apart from the safety-state
store). Full design reasoning: `review/adr
/ADR-021-single-risk-authority-lock.md` §8 (new).

Completed:
- `ReasonCode.RISK_LEDGER_LOCK_UNAVAILABLE` added
- `live_decision.py`/`execution.py` lock-block failure handling
- ADR-021 §7 (AG-023 marked CLOSED) and §8 (AG-024, new)
- `review/INTEGRATION_NOTICES.md` entry for the new `ReasonCode`
...
- logs: `uv run ruff check .` clean; `uv run ruff format --check .` 210
  files already formatted; `uv run mypy` clean, 193 source files
- metrics: full suite, isolated single run (no concurrent DB access),
  real PostgreSQL against `crumblr_test_dev1`: **1438 passed, 3 skipped,
  0 failed, 483.45s**
- artifact/commit: (this commit; pushed via the established
  `core/flatten-close` fast-forward pattern, SHA filled in by the
  follow-up commit)

Problems found:
- none beyond the gap AG-024 itself describes — no new regressions, no
  flaky/contended runs this time (isolated run, no other `pytest`
  process hitting the same database concurrently)

Risk impact:
- closes a real crash-on-transient-database-failure path in both the
  internal live-decision loop and execution-time FINAL Risk revalidation
  — before this fix, a Postgres blip during the lock/recovery/persist
  window would have killed the process outright rather than halting one
  cycle. `order_send` remains unreachable regardless (`feedback.2.0`
  still `NO_TRADE`/`NO-GO`), so this was not a live-safety gap today, the
  same standing caveat as the rest of ADR-021 — it is a Phase-C
  robustness fix.

Decision: fix lives at each caller (wrap the whole `with lock.held(...)`
block), not inside `PostgresRiskLedgerLock.held()` itself — reasoning in
ADR-021 §8.

Next: message Dev 2 with the exact `ReasonCode` name and the
wrap-the-whole-block pattern so `agent_gateway/decision_path.py` and
`application/paper_lite.py`'s two AG-023 call sites mirror the same
shape (`review/INTEGRATION_NOTICES.md`'s new entry already states this
...
## Update 2026-09-08 (ninety-second entry) — Market Universe (ADR-022) merged to `main`

```text
Component: main (merge only — no new code this entry)
Milestone: owner review of 84e5c19 — "APPROVED / merge-ready from owner review. No further Market Universe corrective requested."
Status before: dev1/market-universe pushed, three owner corrective rounds resolved, not merged
Status after:  merged to main via fast-forward, main HEAD = 84e5c19
```

Owner authorized the merge directly ("Merge only after owner
authorization; after merge, report resulting main SHA" — read together
with "APPROVED / merge-ready" as the authorization itself, not a
standing caveat withholding it). Verified `origin/main` had not diverged
from the branch's fork point (`1971d7a`, confirmed via `git merge-base
--is-ancestor origin/main HEAD`) — a clean fast-forward, not a merge
commit: `git push origin dev1/market-universe:main`.

**main HEAD is now `84e5c19`.** Twelve commits land: the five original
Market Universe slices (`AssetClass`, per-market broker-symbol pin,
risk/execution overrides; Crumblr-owned fail-closed broker-symbol
resolution; asset-class-aware trading calendars; per-market risk ledger;
ADR-022/D-060/status.md write-up), plus three owner-requested corrective
rounds (real wiring of `risk_for()`/`execution_for()`/`calendar_for()`
into every real orchestrator plus the `phase_at` fail-closed correction;
cross-market capsule-routing fix in `ExecutionOrchestrator.run_once()`/
`_process()`; reconciliation-history and order-frequency scoping in
`reconcile_once()`/FINAL Risk, plus the identical bug fixed in
`scripts/reconcile.py`). Full narrative across §13 eighty-eighth through
ninety-first entries; full design record in
`review/adr/ADR-022-market-universe.md`; deviation record in
`review/DEVIATIONS.md` D-060.

**Evidence this merge is safe** (no new verification performed this
entry — carried forward from the ninety-first entry's own clean run,
against the exact commit that just became `main`): ruff/format/mypy
clean (201 source files), unit suite 1253 passed/1 skipped, integration
suite 271 passed/2 skipped/0 errors, single Alembic head (`8801080869a6`).
`config/paper.yaml`'s BTC/USD entry stays `enabled: false`; `order_send`
stays NO-GO (`ExecutionConfig.feedback_2_0_approved` still `false`
everywhere) — this merge changes Core's *capability*, not what is
currently authorized to trade.

**What is still open, not silently resolved by this merge** (unchanged
from ADR-022 §4): (1) no owner-approved session policy exists yet for
any non-FX/METAL asset class — every such market fails closed by
construction until one does; (2) the BTC/USD fixture's real-terminal
confirmation via `scripts/mt5_probe.py` is still outstanding; (3)
concurrent multi-market orchestration in one process remains a separate,
future operational change, not attempted here.

**Decision:** merged as authorized. `dev1/market-universe` is now fully
absorbed into `main` — no further work queued on this branch specifically;
it can be deleted once the owner confirms nothing else needs it (not
done unilaterally this entry).

**Next:** idle pending a new work order, or a hosted CI confirmation of
this exact merge commit (no `gh`/Actions access in this environment —
the local re-run evidence above is this session's own substitute, not a
replacement for a real hosted run, same standing caveat as every prior
merge entry).
...
## Update 2026-09-08 (ninety-fourth entry) — BTC/USD Enablement Readiness (ADR-023) merged to `main`

```text
Component: main (merge + one doc-only follow-up commit)
Milestone: owner GO — "GO voor merge van dev1/market-universe op SHA 336e830"
Status before: dev1/market-universe pushed at 336e830, not merged
Status after:  merged to main via fast-forward, main HEAD = 336e830, this entry's own follow-up commit records it
```

Owner authorized the merge directly, with an explicit step list. Verified
`origin/main` had not diverged from the branch's fork point (`966bf6a`,
confirmed via `git merge-base --is-ancestor origin/main HEAD`) — a clean
fast-forward, not a merge commit: `git push origin dev1/market-universe
:main`.

**`main` HEAD is now `336e830`.** Two commits land on top of the
previously-merged Market Universe base (`966bf6a`): the BTC/USD
session-policy implementation (ADR-023) and its documentation record.

**Post-merge regression gate, re-run against `main` @ `336e830` per the
owner's explicit instruction** (not merely re-citing the pre-merge run):
- `uv run ruff check . && uv run ruff format --check .` — clean
- `uv run mypy` — clean, 201 source files
- `uv run pytest --ignore=tests/integration` — **1271 passed, 1 skipped**
  (pre-existing, unrelated MetaTrader5-import skip)
- `uv run pytest tests/integration/test_market_universe_wiring.py` —
  **11 passed**, real PostgreSQL, real fake-MT5 terminal
- `uv run alembic heads` — single head, unchanged (`8801080869a6`)
- The full integration suite (all files, not just the BTC/USD-relevant
  one) had already been run clean against this identical commit content
  immediately before the merge — **273 passed, 2 skipped, 0 errors**,
  see the ninety-third entry — not re-run a second time in immediate
  succession, to avoid repeatedly hammering the single real-Postgres
  container into the connection-pool flakiness already documented
  several times this session (ADR-022 §0a/§0b, ADR-023 §5)

**Confirmed unchanged by this merge, per the owner's own explicit
checklist:**
- `MarketConfig.enabled` — still `false` for BTC/USD in
  `config/paper.yaml`.
- `MarketConfig.expected_spec_version` — still `null`. The real
  `InstrumentSpec` observation in ADR-023 §3 is evidence from real-
  terminal validation, not yet an owner-reviewed-and-accepted pin
  (F-055's own discipline).
- `ExecutionConfig.submission_enabled`/`feedback_2_0_approved` and
  `order_send` reachability — all unchanged, platform-wide, still NO-GO.

**What did change:** `MarketConfig.session_policy_approved: true` for
BTC/USD only — the owner's real, explicit decision (BTC/USD trades
continuously, no weekly close or flatten deadline), per-market not
per-asset-class, closing ADR-022 §4 item 1 for this one market. Real-
terminal broker-symbol validation (§3 of ADR-023) is complete —
`broker_symbol: BTCUSD` confirmed against the live Pepperstone DEMO
terminal, read-only/non-sending throughout.

**Follow-up documentation commit** (this entry's own): ADR-023's status
line and §7 no longer say "Not merged" — updated to record the merge SHA,
the post-merge regression-gate results above, and the explicit "next
Dev-1 work is not enabling BTC/USD" sequencing note. `status.md`'s
compact header `main HEAD` row updated to match.

**Decision:** merge as authorized; do not pin `expected_spec_version` or
flip `enabled` in this pass — both stay separate, later, owner-only
decisions exactly as instructed.

**Next, per the owner's own explicit sequencing — not "turn BTC/USD
on":** the owner must first review the real BTC/USD `InstrumentSpec`
observation in `review/adr/ADR-023-btc-usd-session-policy.md` §3 and
explicitly accept or reject it. Only after that does the BTC/USD Market
Capability Matrix (build.md §24) follow. Idle on this specifically until
that owner review happens or a new work order arrives.

---

## Update 2026-09-08 (ninety-fifth entry) — Dashboard Live Ops work order merged to `main`

```text
Component: main (fast-forward merge, no squash)
Milestone: owner GO — "GO voor merge van dev2/dashboard-live-ops op SHA 3e19bfb", plus one
  owner-requested hardening commit landed the same session before the merge
Status before: dev2/dashboard-live-ops pushed at 398b2e9 (3e19bfb + hardening), not merged;
  main at 9ebbe6f, no dashboard code anywhere on main
Status after:  merged to main via fast-forward, main HEAD = 398b2e9, this entry records it
```

Ten-slice Operational Dashboard vNext work order, built on its own branch
off `dev1/dashboard-current-state` (never itself merged) and only now
landing on `main` — the read-only dashboard package
(`src/crumblr/dashboard/**`, `scripts/run_dashboard.py`) did not exist on
`main` before this. Verified `origin/main` had not diverged from the
branch's rebase base (`9ebbe6f`, confirmed via `git merge-base --is-ancestor
origin/main origin/dev2/dashboard-live-ops`) — a clean fast-forward, 13
commits kept individually, no squash:

- **Slices 1-3** (`762053f`): dashboard heartbeat header card, broker
  account/positions/pending-orders panel.
- **Slice 4** (`c0a9451`): live broker-panel refresh (the panel previously
  froze on first paint, never touched again by `applyState()`) + the
  unified Live Activity feed with client-side filters.
- Corrective (`c8f12ba`, owner review of `1d0687e`): browser `stateClass()`
  brought to exact parity with `app.py::state_class()`'s three explicit
  sets; Account/Terminal trade-allowed rendered as two independent facts
  rather than one collapsed "trade allowed" — `terminal_trade_allowed:
  None` reads `UNKNOWN`, never a silent `YES`.
- **Slice 5** (`4ec2e4d`): the work order §16 8-stage decision pipeline
  (`dashboard/pipeline.py`) — `build_pipeline_view()` is a pure
  presentation layer over the already-identity-bound
  `agent_state.py::LastDecisionState`/`AgentPanelState` evidence, no new
  evidence-gathering. Replaced an older ad hoc inline-Jinja pipeline
  section that had never been wired into the poll refresh at all and did
  not correctly cover every real outcome shape (`SESSION_BLOCKED`,
  `PAPER_ORDER_CHECK_BLOCKED`, `AWAITING_OUTCOME`).
- **Rebase** (`1ead1ab`): synced onto `main`'s Market Universe (ADR-022) +
  BTC/USD (ADR-023) commits, which had landed on `main` independently
  while this branch was in flight. Zero textual conflicts (near-disjoint
  file sets; `status.md`'s only overlap was both sides purely appending).
  One real mechanical break `mypy` caught, not conflict resolution:
  `risk.session.RiskSessionStore.load_latest()` gained a required
  `canonical_symbol` keyword (Market Universe, ADR-022 slice 4) —
  `dashboard/risk_panel.py` was the one caller left passing none, fixed to
  thread the dashboard's own already-configured `canonical_symbol` through.
  Confirmed at the time: nothing in the dashboard package reads
  `session_policy_approved`/`AssetClass`/any Market Universe concept, so
  there was nothing that could infer "CRYPTO approved" from asset class
  alone.
- **Paper portfolio reducer** (`da4e143`): `dashboard/paper_portfolio.py`
  adapts `persistence.paper_lite.PaperPortfolioView` (already documented as
  "a strategy-neutral read model suitable for a dashboard adapter") for the
  dashboard — no new evidence source, no new P&L computation; the real
  numbers come from replaying PAPER_LITE's own durable journal through the
  same `DurablePaperBroker`/`SimulatedBroker` fill engine its own runner
  uses. Two fail-closed gates, both new: never constructs
  `DurablePaperBroker` unless the journal file already exists (that
  constructor writes a fresh header to disk otherwise — the exact side
  effect `dashboard/paper_lite_journal.py` was already built to avoid for
  the activity feed); never replays against a spec that is not the
  owner-approved `expected_spec_version` pin (F-055), since a stale/
  unpinned spec could silently reconstruct different historical fills.
  Explicitly labeled "PAPER_LITE — simulated, not the real broker" in the
  UI. This is exactly why BTC/USD's own `expected_spec_version: null`
  correctly keeps its paper-portfolio numbers `DEGRADED` rather than
  showing a guessed figure — confirmed, not "fixed," during the smoke test.
- **Visual redesign** (`4750236`): CSS-only pass (plus two additive class
  names, no id/structural change, so nothing the test suite or the JS
  refresh logic keys off moved) — a distinct dashed-violet
  `card-simulated`/`simulated-ribbon` treatment so the Paper Portfolio
  section is unmistakable from the real Account & Broker State section
  even at a glance; consistent card elevation; clearer section-to-section
  spacing.
- **Live market smoke test** (`40da876`): proved the dashboard against
  real, current read-only evidence — installed the `mt5` extra locally
  (`uv sync --extra mt5`), ran `scripts/mt5_live_reader.py` against the
  already-logged-in real Pepperstone DEMO terminal (`terminal64.exe`,
  account `62137706`) into `crumblr_soak`, then polled a real
  `scripts/run_dashboard.py` instance across 6+ real cycles: `tick_count`
  climbed monotonically (253018→253182), real varying EUR/USD bid/ask,
  the real broker account showing `account_trade_allowed: true` /
  `terminal_trade_allowed: false` (AlgoTrading genuinely off at the
  terminal) rendered as two independent facts exactly as the earlier
  corrective intended, the platform's real `HALTED` safety state shown
  honestly rather than hidden. Found and fixed one real, observed crash:
  `mt5_live_reader.py --json var/...` raised an uncaught
  `FileNotFoundError` the first time `var/` did not exist yet in a fresh
  worktree — one-line fix (`path.parent.mkdir(parents=True,
  exist_ok=True)`), matching the same convention every other durable
  writer in this codebase already follows.
- **Reader-heartbeat/liveness corrective** (`3e19bfb`, owner-flagged
  truthfulness blocker from the smoke test above): the frozen
  `reader-health` file problem — once `mt5_live_reader.py` exits, its
  last-written `status` never updates again, so the dashboard kept
  reporting `CONNECTED`/`HEALTHY` indefinitely purely because that was the
  last thing the file said. New producer-liveness evidence, distinct from
  market-tick freshness (a live reader can legitimately see no new ticks
  for a while): `ReaderHealth.heartbeat_at_utc` is stamped by
  `LiveReader.poll_once()` on literally every exit path, and
  `heartbeat_max_age_seconds = max(poll_interval, stale_after) × 3`
  travels with every snapshot so the dashboard never invents its own
  freshness threshold. `dashboard.state._connectivity()` now forces
  connectivity to `DISCONNECTED` and raises the data-feed state to at
  least `STALE` whenever the heartbeat is missing or expired — reusing the
  existing state vocabulary exactly as instructed, combined via a small
  severity table so an expired heartbeat only ever makes the picture more
  cautious, never softening an already-`DOWN` reading back to `STALE`.
  Proven end-to-end against the real terminal, not just synthetic tests:
  reader started (`--poll-interval 2 --stale-after 5`, so
  `heartbeat_max_age_seconds: 15.0`) → dashboard read `CONNECTED`/`HEALTHY`
  while it ran → reader exited cleanly → ~20s later, the same
  never-restarted dashboard process read `DISCONNECTED`/`STALE`, while the
  raw health file on disk still literally said `"status": "HEALTHY"`.
- **Hardening** (`398b2e9`, owner-flagged, non-blocking): a syntactically
  valid but naive (no timezone offset) `heartbeat_at_utc` would have raised
  an uncaught `TypeError` subtracting a naive datetime from the always-
  timezone-aware `now` — moved inside the existing malformed-timestamp
  `try`/`except` so it now fails closed to expired instead of crashing.

**Unchanged, explicitly confirmed by this merge:** the dashboard remains
fully read-only — `TestReadOnlyBoundary` (no route accepts a mutation, the
dashboard package never imports `MetaTrader5`/`crumblr.mt5_gateway`, `POST
/` refused) re-run clean at every slice; only `scripts/mt5_live_reader.py`,
a separate process, ever touches real MT5 credentials, never dashboard
code itself; `ExecutionConfig.submission_enabled`/`feedback_2_0_approved`/
`order_send` all stay NO-GO, platform-wide, untouched by any of this work.

**Post-merge regression gate, re-run against `main` @ `398b2e9`:**
- `uv run ruff check . && uv run ruff format --check .` — clean
- `uv run mypy` — clean, 221 source files
- `uv run pytest --ignore=tests/integration` — **1343 passed, 1 skipped**
  (pre-existing, unrelated MetaTrader5-import skip)
- `uv run pytest tests/integration -m integration` — **332 passed, 2
  skipped** (both pre-existing, unrelated filesystem-permission skips),
  real PostgreSQL — the whole platform (Core, Agent Gateway, PAPER_LITE,
  Market Universe, Dashboard) green together on `main` for the first time
- `uv run alembic heads` — single head, unchanged (`8801080869a6`)

**Decision:** merge as authorized, fast-forward, no squash — every slice's
own commit and evidence trail stays intact and individually reviewable.
...
## Update 2026-09-16 (ninety-eighth entry) — CRUMBLR DEMO EXECUTION CHAPTER: FINAL Risk defect fix, first real MARKET round-trip, ICT LIMIT Slice 1, real ICT watcher — owner-declared engineering-complete

```text
Component: risk/policies.py, mt5_gateway/{client,execution,demo_execution,readonly,port,simulated}.py, application/{execution,execution_outcome,expected_state}.py, domain/models.py, persistence/{schema,broker_state}.py, scripts/{close_demo_canary_position,cancel_demo_canary_pending_order,issue_canary_permit}.py, migrations/versions/20260915_fb1abaa2d137_broker_pending_order_snapshots_magic.py
Milestone: M5 paper execution — first real order round-trips, real ICT LIMIT execution path
Status before: FINAL Risk revalidation compared carried risk against evaluate()'s own rounded sizing artifact, not the true ADR-001 budget; no real MT5 order had ever been sent; LIMIT entries were structurally unsupported (MARKET-only order construction, no pending-order magic tracking, D-049 open)
Status after: FINAL Risk correctly compares against current_equity*requested_risk_fraction; a full real MARKET entry+close round-trip has executed against Pepperstone DEMO; real ICT LIMIT submission/cancellation/resolution is built, reviewed (3 rounds) and merged; a real ICT LIMIT watcher has run continuously against the real Static Agent since, with zero broker mutation
```

This entry consolidates a long chapter run across several real-time
work orders, all under `CLAUDE.md`'s no-live/no-secrets-in-status
discipline throughout. Written as one entry per the owner's own
"CRUMBLR DEMO EXECUTION CHAPTER — ENGINEERING COMPLETE" closure message
rather than reconstructed retroactively slice-by-slice.

**1. FINAL Risk budget defect — found, evidenced, fixed.**
`risk/policies.py::revalidate_fixed_volume_at_execution_time()` compared
carried risk against `fresh.risk_amount` — which `evaluate()` actually
sets to `realised_risk(sizing.volume, ...)`, the *rounded-down realized
risk of the approved volume*, not the ADR-001 budget ceiling
(`current_equity * intent.requested_risk_fraction`). Real attempts 3/4/6
proved the two diverge whenever `volume_step` doesn't evenly divide the
raw budget-implied volume (attempt 6: `fresh.risk_amount=7.00 <
carried <= current_budget=8.0000`) — a materially looser effective ceiling
than ADR-001 specifies. Minimal fix: compare `carried` against
`current_budget` computed fresh from `fresh_portfolio.account.equity *
intent.requested_risk_fraction`, preserving every other FINAL Risk
semantic. Seven new tests
(`TestExecutionTimeComparesAgainstTheRealBudgetNotAFreshSizingArtifact`)
cover adverse/favourable moves and equity drops against a fixture
deliberately chosen not to divide cleanly. Reviewed PASS, merged
`6fa6066e50b9abfd6328735b391b6b17fcb4a261`.

**2. First real Pepperstone DEMO order round-trip.** Attempt 7 (BUY,
`requested_risk_fraction=0.0005`, sl=200pts/tp=400pts, the same canary
assignment/artifact as prior attempts, a fresh one-shot durable permit,
`config/agent_canary_demo.yaml`'s scoped `submission_enabled`/
`feedback_2_0_approved` override) reached a real MT5 fill — the first
time this platform has ever sent and had a broker accept an order.
Closed explicitly and deliberately: a minimal one-shot close runner
(`scripts/close_demo_canary_position.py`) built around the
already-reviewed `DemoOrderSendMt5Gateway.close_position()`, never
automatic flatten, never a raw `order_send` call. One real review
blocker found before execution: `Mt5CallFailedError` short-circuited
before the mandatory post-close `positions()` readback, so a
transport-ambiguous close would have skipped verification entirely —
fixed (`_decide_outcome(result, transport_error, *, readback_flat)`
always takes the fresh readback first), 6 new regression tests
(`TestDecideOutcome`), re-reviewed PASS. Executed against the real
authorized target (ticket `89517316`): the close succeeded and was
verified via a fresh, independent `positions()` read showing the ticket
gone — **the first complete real MARKET entry+close cycle this
platform has ever run.**

**3. ICT LIMIT DEMO EXECUTION Slice 1** — making the real external
Static ICT Agent's genuine `LIMIT` intents executable, LIMIT-only (no
STOP), reusing the existing Agent → Gateway → Risk → Policy →
Supervisor → Execution → Reconciliation chain unchanged. Built per an
explicit execution-first-scoped plan (Slice 1: real LIMIT submission +
pending-order magic tracking + a minimal SUBMITTED→FILLED resolver;
Slice 2 — cancel/expire/reject disambiguation via
`history_orders_get()` — deliberately deferred). Key pieces:
`mt5_gateway/execution.py::build_order_request()` branches on
`EntryType.MARKET` (`TRADE_ACTION_DEAL`/`ORDER_FILLING_IOC`) vs
`EntryType.LIMIT` (`TRADE_ACTION_PENDING`/`ORDER_FILLING_RETURN`);
`PendingOrderState.magic`/`BrokerPendingOrderSnapshot.magic` (migration
`fb1abaa2d137`) give pending orders the same MT5-magic provenance
positions already had; `_recover_ambiguous_submission()` now also
searches `pending_orders()` by magic; a new
`ExecutionOrchestrator.resolve_pending_once()` resolves
`SUBMITTED`→`FILLED`; `scripts/cancel_demo_canary_pending_order.py`
mirrors the close runner's exact-ticket pattern for pending cancellation
(never the plural `cancel_pending_orders()`, never raw `order_send`).
**Three Dev-1 review rounds, each a real defect found and fixed, not a
style note:** (a) pending LIMIT orders were being submitted with
`ORDER_FILLING_IOC` (market-fill-only semantics) instead of
`ORDER_FILLING_RETURN` — fixed, new `Mt5Module` constant added; (b)
`resolve_pending_once()` v1 inferred FILLED from a magic-matched
position alone, without first confirming the exact submitted pending
ticket was no longer resting — a real correctness gap (could have
falsely marked a still-pending order FILLED) — fixed to recover the
durable `mt5_order_ticket` from the `SUBMITTED` event's own payload and
require it be confirmed absent from a fresh `pending_orders()` read
first; (c) the final blocker asked for the missing exact-ticket cancel
runner, built as described above. A proactive safety fix made alongside
review round (b), not explicitly requested: `application/
expected_state.py::derive_expected_exposure()` gained a `SUBMITTED`
branch so post-fill reconciliation never silently reads a resting
pending order as a false `MATCHED`. Merged
`9553a5843309137eed69c514e415e6dbb0b0a5ad`. Migration `fb1abaa2d137`
applied to the real `crumblr_soak` database ahead of runtime use.

**4. Real ICT LIMIT runtime watch.** After re-verifying the real ICT
Agent/assignment/artifact identity from the live registry (never from
memory — the registered agent's own `service_identity`/
`runtime_version` literally read "toy agent," a naming discrepancy
flagged honestly rather than silently accepted or treated as a
blocker, since status/artifact-hash/assignment-window all confirmed it
usable), a persistent detached background watcher
(`real_ict_limit_watcher.py`, matching this project's established
genuinely-detached-process pattern for long-running daemons) has run
one preflight-only real Agent/Gateway/Risk/Policy/Supervisor decision
cycle per newly-closed real EUR/USD M5 bar continuously since
2026-09-15, against the real `crumblr-static-agent-host` process
(artifact hash `81894d6a9c44ddb0433c15f9779fb0a2e25c0e1eccee232e2fd145d72cb498c5`).
No permit is ever issued in this mode; `order_send` is structurally
unreachable from every cycle. A real bug in the watcher itself was
found and fixed while this was running (self-applying this project's
own "never trust a single log line" discipline): capsule-detection
originally compared a new capsule's `occurred_at_utc` (market/tick
time) against a wall-clock `datetime.now()` threshold, and since tick
time can trail wall-clock time by a fraction of a second, two
genuinely-sealed capsules were logged as "no capsule found (unexpected)."
Both were independently verified by hand against the database before
any fix — both were genuinely `NO_TRADE`, nothing was actually missed.
Fixed by switching to `decision_capsules.sequence` (a database-assigned,
monotonic write-order identity) as the watermark instead of wall-clock
time. As of this entry: dozens of real cycles observed, **every one a
database-verified genuine `NO_TRADE`** against the correct artifact
hash, broker book confirmed flat on every cycle, no `FOUND_MARKER`, no
`STOPPED_MARKER`.

**Evidence:**
- FINAL Risk fix: 7 new unit tests, full quality gate clean, reviewed PASS
- Close runner: 23 unit tests (6 new for the transport-ambiguity fix),
  real execution against ticket `89517316` verified via fresh
  independent `positions()` readback
- ICT LIMIT Slice 1: full new/changed test surface across
  `tests/unit/test_mt5_execution_gateway.py`,
  `test_demo_order_send_gateway.py`, `test_mt5_readonly_gateway.py`,
  `test_mt5_probe.py`, `test_live_reader.py`, `test_risk_engine.py`,
  `test_canary_permit.py`, `test_execution_outcome.py`,
  `test_expected_state.py`,
  `tests/integration/test_execution_orchestrator.py` (+4),
  `tests/integration/test_execution_reconciliation.py`
  (`TestResolvePendingOnce` + regressions),
  `tests/unit/test_cancel_demo_canary_pending_order.py` (18, new) — full
  quality gate clean each review round, reviewed PASS v3
- Real ICT watcher: direct database verification of every "unexpected"
  log anomaly before touching code; direct database query at this
  entry's own writing confirms **57/57 capsules sealed since the watch
  began carry `trade_intent = null`** — zero proposals of any kind, not
  merely zero LIMIT ones

**Problems found:** the FINAL Risk contract mismatch (real defect,
fixed); the close runner's transport-ambiguity short-circuit (real
defect, fixed before execution); the LIMIT filling-mode mismatch (real
defect, fixed); the `resolve_pending_once()` premature-FILLED logic gap
(real defect, fixed); the watcher's own timestamp-vs-tick-time capsule
lookup (real defect in this session's own tooling, fixed, verified
nothing was actually missed).

**Risk impact:** none of this widens platform-wide execution
authority — `config/paper.yaml`'s `feedback_2_0_approved`/
`submission_enabled` stay `false`; both real order_send events to date
ran only under the dedicated, explicitly owner-scoped
`config/agent_canary_demo.yaml`. No LIVE activation anywhere in this
chapter.

**Decision:** owner declares the CRUMBLR DEMO EXECUTION CHAPTER
engineering-complete (§10 decision log, 2026-09-16). The outstanding
genuine ICT LIMIT trade is now an acceptance-observation item, not
unfinished Crumblr engineering. Dev 2 leaves the watcher running
unmodified pending a genuine accepted LIMIT proposal; Dev 1 owes no
further review on this chapter absent a concrete Core/execution
defect.

**Next:** watcher continues, one cycle per real M5 bar, indefinitely;
report `REAL ICT LIMIT DEMO — PROPOSAL READY` the moment a genuine
LIMIT proposal passes the full chain and seals a capsule — verified
directly against the database before ever being reported, never from
the log alone. Active development moves to the next chapter.
