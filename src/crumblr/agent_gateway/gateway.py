...
Owns exactly the responsibilities `CRUMBLR_DEV2_AGENT_INTEGRATION_INSTRUCTIONS_V2.md`
§7 lists for Step B: service identity/authentication, assignment
...
platform `TradeIntent`** — see `review/AGENT_STATUS.md` AG-006: `TradeIntent`
requires a `feature_snapshot_id`, and deciding what that means for an
externally-originated decision is a shared-contract question this pass
deliberately did not resolve alone (`CRUMBLR_DEV2_AGENT_INTEGRATION_INSTRUCTIONS_V2.md`
§4/§5 — stop and raise, don't force it). What this module proves instead is
everything ADR-005 §8's "first proof target" actually requires: identity,
assignment, context and outcome durably recorded in SHADOW, zero broker
execution, zero calls into `crumblr.application.execution`/`mt5_gateway`/the
platform database write path other than this package's own tables.
...
class AgentGateway:
        ...
        *,
        ...
        count_before = (
            ...
            if assignment is None
                ...
                assignment_id=assignment.assignment_id, since=now - _RATE_LIMIT_WINDOW
            )
        )

        claim = self._outcomes.claim_trade_proposal(proposal, now=now)
        if not claim.claimed:
            return self._replay(proposal.proposal_id, AgentOutcomeType.TRADE_PROPOSAL)
        ...
        return self._settle(proposal.proposal_id, AgentOutcomeType.TRADE_PROPOSAL, reason, now=now)
    ...
    def _evaluate_proposal(
        ...
        *,
        ...
        if count_before is not None and count_before >= assignment.max_proposals_per_hour:
    ...
    def submit_no_trade(
        ...
        *,
        ...
        claim = self._outcomes.claim_no_trade(decision, now=now)
        if not claim.claimed:
            return self._replay(decision.decision_id, AgentOutcomeType.NO_TRADE)
        ...
        return self._settle(decision.decision_id, AgentOutcomeType.NO_TRADE, reason, now=now)
    ...
    def _evaluate_no_trade(
        ...
        *,
        ...
        bundle = self._contexts.by_hash(decision.context_hash)
        if bundle is None:
            return AgentRejectionReason.UNKNOWN_CONTEXT
        if bundle.assignment_id != assignment.assignment_id:
            return AgentRejectionReason.CONTEXT_ASSIGNMENT_MISMATCH
    ...
    def _settle(
        ...
        *,
        ...
        self._outcomes.append_event(
            outcome_id=outcome_id, event_type=AgentDecisionEventType.ACCEPTED, occurred_at_utc=now
        )
        return AgentDecisionOutcomeResult(
    ...
    def _replay(
        self, outcome_id: UUID, outcome_type: AgentOutcomeType
    ) -> AgentDecisionOutcomeResult:
        """An identical retry of an already-claimed outcome — read back what

        was already durably decided rather than re-running any check, so a
        retry can never observe a different verdict than the original call
        did (e.g. because the rate-limit window has since moved)."""
        events = self._outcomes.events_for(outcome_id)
        rejected = next(
            (event for event in events if event.event_type is AgentDecisionEventType.REJECTED), None
        )
        if rejected is not None:
            reason = (
                AgentRejectionReason(rejected.reason_codes[0]) if rejected.reason_codes else None
            )
            return AgentDecisionOutcomeResult(
                outcome_id=outcome_id, outcome_type=outcome_type, accepted=False, reason=reason
            )
