...
| F-033 | MEDIUM | `status.md` current-state sections still lagged behind the first-contact update | **REOPENED 2026-08-24 by review 1.10, CLOSED, PARTLY REOPENED again 2026-08-24 by review 1.11, then CLOSED again the same day** | N/A — documentation | 1.9's fix was itself incomplete — the MT5 checklist, the M1 milestone row, "Overall health" and the repo/build checklist (stale `(no remote)`, stale test/file counts) all still described the reader as unbuilt after it had shipped. Rewritten to distinguish impl+unit-tested from real-terminal-validated per capability. Review 1.11 found a third instance of the same defect class: §12 "Next 10 actions" still listed the Pepperstone entity and Q2 hedging/netting as open checkboxes after both had been recorded resolved (O-005, `RETAIL_HEDGING`) elsewhere in the same document. Both items checked off in place; test counts refreshed (705 passed, 3 skipped, after the D-040/D-041/F-031/F-038 cycle) |
| F-034 | HIGH | Reconnect must revalidate broker/account truth before data flow resumes | OPEN — reconfirmed by review 1.11 | SHIPPED, unit-tested only | `src/crumblr/application/live_reader.py::LiveReader`. All five required scenarios pass against a scripted fake terminal (`tests/unit/test_live_reader.py`, 16 tests): normal reconnect recovers, wrong account fails closed and stays closed until `acknowledge()`, a changed symbol spec is detected and recorded without silent continuation, silence past threshold becomes STALE and self-clears, every reconnect re-runs the full account guard. **Not yet run against a real terminal.** Review 1.11 §2: "unit evidence remains strong, but Phase B is still mandatory" — closes only after the owner-present deliberate terminal interruption |
| F-035 | HIGH DESIGN | UI v0 must remain read-only and outside the broker execution boundary | OPEN UNTIL BUILT | PENDING | Hard boundary specified: reads PostgreSQL/app state only, no `MetaTrader5` import, no credentials, no `order_send`, no HALT reset, no risk-config write, no `TradeIntent` creation, no buttons. Not yet built. Review 1.11 §9 reconfirms it may start after Phase A produces real rows, in parallel with Phase B preparation — not before, not instead |
...
| F-037 | HIGH | MT5 timestamp semantics must be verified against the real feed before M1 is qualified | OPEN — reconfirmed by review 1.11 | PENDING REAL SOAK | Same open question as D-039. Cannot close from code — needs the soak test's comparison of stored `event_time_utc`/`received_time_utc` against wall-clock reality, expected M5 boundaries and ordering across polls (review 1.11 §7) |
...
## Unreviewed work — `feedback.1.12.md` not yet triggered

`feedback.1.7.md` through `feedback.1.11.md` have all been processed (F-026…
F-038, O-005, above). Review 1.11 §12 step 14 names the trigger for
`feedback.1.12.md`: M1 qualification / dashboard review, once the real soak
(Phase A clean 30-60 minute run, Phase B deliberate interruption) has
produced recorded evidence.
...
| D-042 (new, found on the third real soak) | `ReadOnlyMt5Gateway.bars()` was persisting MT5's current, still-forming bar as though it were closed; its close moved between polls and `record_bars` correctly read that as a contradiction and halted. Fixed by excluding any bar whose interval has not yet closed | `review/DEVIATIONS.md` D-042; `src/crumblr/mt5_gateway/readonly.py::bars`; `tests/unit/test_mt5_readonly_gateway.py::TestBars` (+2) |
| CI | Still not confirmed running — unchanged from the 1.10/1.11 entries | — |
| The real soak (Phase A/B) | **Not yet run to completion** as of this entry — the reviewer's stated top priority, both in 1.10 and again in 1.11. Everything above is preparation for it, not a substitute | — |

`feedback.2.0.md` remains mandatory before the first `order_send`, demo
included, and is separate from 1.11.
