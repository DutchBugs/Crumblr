...
| ~~Read-only MT5 gateway~~ | M1 | **Done** 2026-08-24 — written 2026-08-23, first contact 2026-08-24, Phase A (clean 30-minute soak) and Phase B (two real reconnects, owner present) both completed the same day. `mt5_gateway/readonly.py` + `scripts/mt5_probe.py` + `application/live_reader.py`. M1 qualification itself is still a reviewer/owner decision |
...
## Unreviewed work — `feedback.1.12.md` now triggered
...
produced recorded evidence. **Both have now happened** — Phase A 2026-08-24
(sixth attempt, clean), Phase B 2026-08-24 (owner present, two deliberate
terminal closures, both recovered). The trigger condition is met; M1
qualification itself remains a reviewer/owner decision, not something this
tracker declares on its own.
