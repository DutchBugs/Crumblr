...
| ~~Read-only reconciliation v0~~ | M5 | **Done** 2026-08-25 — `application/reconciliation.py`, `scripts/reconcile.py`, F-047's snapshots as the observed side, `ExpectedState.flat()` as the (pre-execution) expected side. Real-terminal evidence still required (F-051) before this counts toward M5 |
...
## Unreviewed work — `feedback.1.17.md` now triggered

`feedback.1.7.md` through `feedback.1.16.md` have all been processed
(F-026…F-052, O-006, above). **M1 remains PASSED, M2 remains PASSED**
(`status.md` §16 Promotion history); Dashboard v0's visual scope stays
**frozen**. Review 1.16 §17's preferred trigger for the next regular review
is a meaningful bundle — real F-047 validation + reconciliation + CI/M0
closure, and/or first live-shadow Agent decisions on real M5 — but also
explicitly welcomes a real integration defect as its own trigger.

Pruned per the standing instruction (review 1.13 §13) — everything from
reviews 1.7 through 1.15 has now been seen by a reviewer (1.15 and/or 1.16
...
|---|---|---|
| F-047 (reconfirmed by 1.16 §2, "IMPLEMENTED AND PERSISTENCE-TESTED") | Durable broker account/position/pending-order snapshots — see the finding register above | `src/crumblr/application/broker_state.py`, `src/crumblr/persistence/broker_state.py` |
| CI | Still not confirmed running — unchanged since review 1.10; review 1.16 §11 "no longer allowed to drift". Workflow reviewed 2026-08-25 and looks correct; two pushes to `main` since should have triggered it automatically, but this session has no `gh` CLI/Actions access to check the result — needs a human to check and record | `.github/workflows/ci.yml` |
| **Domain-contract package for reviewer approval (assembled 2026-08-25)** | Covers all twelve contracts review 1.14 §13 named, checked against immutability/extra-field-rejection/Decimal-time-semantics/ownership/execution-permissions/risk-supervisor-separation/agent-controlled-fields. Not yet actually reviewed by a human — the package existing is not the same claim as it being approved | `review/domain_contracts.md` |
| **F-050 — `BrokerStateHealth` (new, review 1.16, shipped same day)** | Broker-state freshness kept as its own type, separate from `ReaderStatus`; `is_usable(now, max_age)` encodes the missing/stale/incomplete → UNKNOWN rule directly | `src/crumblr/application/live_reader.py::BrokerStateHealth`; `tests/unit/test_live_reader.py::TestBrokerStateHealthIsUsable` |
| **F-051 — real-terminal verification of F-047 (new, review 1.16)** | Not yet done — no MT5 host available this session. `pending_orders()`/`account_extras()`/the snapshot mapping remain fake-terminal-tested only | — |
| **F-052 — one coherent account observation per snapshot (new, review 1.16, shipped same day)** | `account_with_extras()` replaces two separate `account_info()` calls with one; a dedicated test asserts exactly one call is made | `src/crumblr/mt5_gateway/readonly.py::account_with_extras`; `tests/unit/test_mt5_readonly_gateway.py::TestAccountWithExtras` |
| **Reconciliation v0 (new, built this session per review 1.15 §14/1.16 §7-8)** | Compares the latest F-047 snapshot against `ExpectedState` (flat, pre-execution); `MATCHED`/`MISMATCHED`/`UNKNOWN` with the review's exact fail-closed rule table; `UNKNOWN` never upgraded to `MATCHED`. Real demo account has never been reconciled against (no live capture has run yet — F-051) | `src/crumblr/application/reconciliation.py`, `scripts/reconcile.py`; `tests/unit/test_reconciliation.py` (19), `tests/integration/test_reconciliation.py` (3) |
| **F-048 — live/shadow decision orchestrator (shipped this session)** | `LiveDecisionOrchestrator`: real M5 bar → features → Trading Agent → intent-time Risk → Supervisor → persist, execution unreachable by construction. See the finding register above for full detail | `src/crumblr/application/live_decision.py`, `scripts/live_decision.py`; `tests/unit/test_live_decision.py`, `tests/integration/test_live_decision.py` |
| InstrumentSpec durable persistence (closes part of D-045, unblocked F-048) | `LiveReader` now persists the real spec it already observes on every reconnect, via a new `InstrumentSpecStore` | `src/crumblr/persistence/instrument_specs.py`; `tests/integration/test_instrument_spec_store.py` |
| F-049 — multi-gated execution enablement | Not yet built, correctly — M5 is NO-GO | — |
