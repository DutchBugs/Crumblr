...
| F-053 | HIGH BEFORE M5 | Reconciliation must compare the semantic instrument specification, not only account/position identity | CLOSED — **reviewer instructed "build now" 2026-08-26 (review 1.18 §6), shipped same day** | SHIPPED | `application/reconciliation.py::reconcile()` now requires an `InstrumentSpecSource` (`latest`/`earliest`) and compares the *baseline* spec — the first one ever durably observed for the symbol, since this platform deliberately never hard-codes a contract specification (O-001) so there is no config-declared "expected" spec to compare against instead — against the *current* one, via `InstrumentSpec.spec_version` (already excludes `captured_at_utc`/`tick_value`, F-039). Missing/unreadable spec → `UNKNOWN`; a changed `spec_version` → `MISMATCHED`, combined with any account/position/pending-order mismatches. `InstrumentSpecStore.earliest()` (new). `scripts/reconcile.py` and `LiveDecisionOrchestrator` both wired to pass their spec source through. Tests: `tests/unit/test_reconciliation.py::TestInstrumentSpecReconciliation` (5), `tests/integration/test_reconciliation.py` (+2), `tests/integration/test_instrument_spec_store.py::TestEarliest` (3) |
| F-054 | CRITICAL BEFORE FIRST ORDER | Live decision-window idempotence must survive a restart before an execution path exists | CLOSED — **reviewer termed this a "hard prerequisite," shipped 2026-08-26** | SHIPPED | New `application/decision_window.py::DecisionWindowState`/`DecisionWindowStore` (same shape as `risk/session.py`'s F-019 pattern) + `persistence/decision_window.py::PostgresDecisionWindowStore`, keyed by `(canonical_symbol, strategy_id, config_version)` — matching the review's own invariant ("same strategy + same config + same canonical symbol..."). `LiveDecisionOrchestrator` restores `_last_decided_open_time`/`_seen_hashes` from the store on construction (not blank) and re-saves after claiming a window and again after a hash is added, so a restart cannot re-decide an already-decided window nor forget which `TradeIntent` hashes the risk engine's duplicate-protection check has seen. Deliberately simpler failure semantics than `RiskSessionStore`: an unreadable record collapses to "nothing recorded" rather than halting, since the worst consequence today is a duplicate audit row, not a duplicate order — documented explicitly as a choice to revisit once execution exists. Migration `a7c4e19d6f52`. Tests: `tests/unit/test_live_decision.py::TestF054DurableDecisionWindowIdempotence` (4, including a config-version-changes-the-key case), `tests/integration/test_decision_window_store.py` (6), `tests/integration/test_live_decision.py` (+1, real PostgreSQL restart simulation) |
...
| ~~Read-only reconciliation v0~~ | M5 | **Done** 2026-08-25, instrument-spec comparison added 2026-08-26 (F-053) — `application/reconciliation.py`, `scripts/reconcile.py`, F-047's snapshots plus the durable instrument spec as the observed side, `ExpectedState.flat()`/the spec baseline as the expected side. Real-terminal evidence still required (F-051) before this counts toward M5 |
| ~~Durable live-decision idempotence~~ | before first order | **Done** 2026-08-26 (F-054) — `application/decision_window.py`, `persistence/decision_window.py`. Required before an execution service may be attached, not before M5 itself |
...
## Unreviewed work — `feedback.1.19.md` now triggered

`feedback.1.7.md` through `feedback.1.18.md` have all been processed
(F-026…F-054, O-006, above). **M1 remains PASSED, M2 remains PASSED**
(`status.md` §16 Promotion history); Dashboard v0's visual scope stays
**frozen**. Review 1.18 §15's preferred trigger for the next regular review
is: F-051 real MT5 evidence + F-053 + F-054 + D-031 + the CI result +
`review/domain_contracts.md` actually supplied, arriving together — and
explicitly *not* another documentation-only update. F-053, F-054 and D-031
are now built (see below); F-051, CI and the contract supply remain, all
three genuinely blocked on something this session cannot produce (an MT5
host, `gh`/Actions access, and a human reviewer respectively).

Pruned per the standing instruction (review 1.13 §13) — everything from
reviews 1.7 through 1.17 has now been seen by a reviewer (1.17 and/or 1.18
...
|---|---|---|
| CI | Still not confirmed running — unchanged since review 1.10; review 1.17/1.18 both say this is "no longer an engineering blocker, it is an evidence retrieval task" and ask for commit SHA, Linux job, Windows job, PostgreSQL tests, gitleaks and overall status to be recorded. This session still has no `gh` CLI/Actions access — needs a human to check and record | `.github/workflows/ci.yml` |
| **Domain-contract package for reviewer approval (assembled 2026-08-25)** | Covers all twelve contracts review 1.14 §13 named. Review 1.17/1.18 both state the reviewer has **not** been given the actual file yet and M0 contract review is therefore not approved — supplying it unchanged is a required action, not optional | `review/domain_contracts.md` |
| **F-051 — real-terminal verification of F-047/F-048/reconciliation (reconfirmed by review 1.17 §6/1.18 §5, still a single checklist, now also covering F-053)** | Not yet done — no MT5 host available in any session so far. `pending_orders()`/`account_extras()`/the snapshot mapping/reconciliation/instrument-spec comparison/`LiveDecisionOrchestrator`/decision-window durability/feature persistence all remain fake-terminal-tested only | — |
| **F-053 — instrument-spec reconciliation (built and shipped 2026-08-26, per review 1.18 §6's explicit "build now" instruction)** | `reconcile()` compares the durable spec baseline against the latest observation via `spec_version`; missing/changed → `UNKNOWN`/`MISMATCHED` | `src/crumblr/application/reconciliation.py`, `src/crumblr/persistence/instrument_specs.py::earliest` |
| **F-054 — durable live-decision idempotence (built and shipped 2026-08-26, per review 1.18 §7's "hard prerequisite" framing)** | `LiveDecisionOrchestrator` restores and re-persists its decision-window/duplicate-protection state via a new `DecisionWindowStore`, keyed by strategy+config+symbol | `src/crumblr/application/decision_window.py`, `src/crumblr/persistence/decision_window.py` |
| **D-031 — feature-value persistence (built and shipped 2026-08-26)** | `FeatureSnapshotStore` durably records the full `FeatureEvidence` payload (not only its hash) for every evaluated window, wired into both the replay and live paths via `RunRecorder.record_features` | `src/crumblr/persistence/features.py`, `src/crumblr/application/recording.py` |
| **F-033, sixth reopen/close (2026-08-26)** | Four current-state contradictions in `status.md` corrected (pending-order/reconciliation status, live-decision-pipeline existence, `instrument_specs` producer status, M1 reconnect validation status), found because the document is now positioned as onboarding truth for a second developer | `status.md` §3, current-risks table |
| HANDOVER.md / README.md / status.md documentation pass (previous session, 2026-08-26) | Brought current for a new developer joining the project — HANDOVER.md rewritten against the actual current state; README.md's implemented/not-implemented table and layout section updated; a tracker bookkeeping bug (duplicate F-052 row) found and fixed | `HANDOVER.md`, `README.md`, `status.md` |
