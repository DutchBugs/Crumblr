...
| F-053 | HIGH BEFORE M5 | Reconciliation must compare the semantic instrument specification, not only account/position identity | OPEN | PENDING | Review 1.17 §7: F-048 gave `instrument_specs` a real producer (`LiveReader` → `InstrumentSpecStore`), which removes the reason `application/reconciliation.py::reconcile()` was originally built without a spec comparison — `review/DEVIATIONS.md` D-045 named this exact trigger ("the day `instrument_specs` gets a real producer, add a spec-version comparison"). Required: compare expected vs. current broker symbol, digits, point, tick size, contract size, volume min/max/step, stops level, freeze level, trade mode and filling capability; a material difference is `MISMATCHED`, a missing/unreadable spec is `UNKNOWN`. Must not use the volatile `tick_value` field as a change trigger — F-039 already excluded it from `InstrumentSpec.spec_version` for exactly this reason. Must be complete before `feedback.2.0`. Deferred this session — user's explicit direction was a documentation/handover pass, not further engineering; queued as the next concrete engineering task alongside F-054 |
| F-054 | CRITICAL BEFORE FIRST ORDER | Live decision-window idempotence must survive a restart before an execution path exists | OPEN | PENDING | Review 1.17 §8: `LiveDecisionOrchestrator._seen_decision_hashes` (D-046 point 3) lives only in process memory. Today a restart can at worst produce a duplicate *audit* row, because no order path exists — but the instant an execution service is attached, the same closed M5 window must never become two independently executable order proposals merely because the process restarted. Required invariant: `same strategy + same config + same canonical symbol + same closed M5 decision window + same feature/input identity → same logical decision identity`, held across normal operation, restart, reconnect and ambiguous crash recovery — the platform may return the existing decision or idempotently reconstruct it, but must not create a second executable request. Should connect directly to the later durable `order_request_id`. Deferred this session for the same reason as F-053 — queued as the next concrete engineering task |
...
| ~~Read-only reconciliation v0~~ | M5 | **Done** 2026-08-25 — `application/reconciliation.py`, `scripts/reconcile.py`, F-047's snapshots as the observed side, `ExpectedState.flat()` as the (pre-execution) expected side. Real-terminal evidence still required (F-051) before this counts toward M5. Review 1.17 §7/F-053: still missing the instrument-spec comparison half — open |
| Instrument-spec reconciliation (F-053) | M5 | Open — `instrument_specs` now has a producer (F-048), so the gap D-045 named is now unblocked but not yet closed. Deferred 2026-08-26 in favour of a documentation/handover pass |
| Durable live-decision idempotence (F-054) | before first order | Open — required before an execution service may be attached, not before M5 itself. Deferred 2026-08-26 alongside F-053 |
...
## Unreviewed work — `feedback.1.18.md` now triggered

`feedback.1.7.md` through `feedback.1.17.md` have all been processed
...
**frozen**. Review 1.17 §19's preferred trigger for the next regular review
is: F-051 real Pepperstone validation + a real shadow Agent decision + the
CI result + `review/domain_contracts.md` actually supplied for inspection,
arriving together.

Pruned per the standing instruction (review 1.13 §13) — everything from
reviews 1.7 through 1.16 has now been seen by a reviewer (1.16 and/or 1.17
...
|---|---|---|
| CI | Still not confirmed running — unchanged since review 1.10; review 1.17 §11 says this is "no longer an engineering blocker, it is an evidence retrieval task" and asks for commit SHA, Linux job, Windows job, PostgreSQL tests, gitleaks and overall status to be recorded. This session still has no `gh` CLI/Actions access — needs a human to check and record | `.github/workflows/ci.yml` |
| **Domain-contract package for reviewer approval (assembled 2026-08-25)** | Covers all twelve contracts review 1.14 §13 named. Review 1.17 §10 explicitly states the reviewer has **not** been given the actual file yet and M0 contract review is therefore not approved — supplying it unchanged is a required action, not optional | `review/domain_contracts.md` |
| **F-051 — real-terminal verification of F-047/F-048/reconciliation (reconfirmed by review 1.17 §6, now a single twelve-step checklist)** | Not yet done — no MT5 host available in any session so far. `pending_orders()`/`account_extras()`/the snapshot mapping/reconciliation/`LiveDecisionOrchestrator` all remain fake-terminal-tested only | — |
| **F-053 — instrument-spec reconciliation (new, review 1.17, deferred 2026-08-26)** | Not yet built. `reconcile()` still has no comparison of broker symbol/digits/point/tick size/contract size/volume steps/stops level/freeze level/trade mode/filling capability | `src/crumblr/application/reconciliation.py` |
| **F-054 — durable live-decision idempotence (new, review 1.17, deferred 2026-08-26)** | Not yet built. `LiveDecisionOrchestrator`'s `seen_decision_hashes` remains process-memory only | `src/crumblr/application/live_decision.py` |
| **D-031 — feature-value persistence** | Review 1.17 §9: the exception that allowed F-048's first wiring test without full feature-value persistence "has served its purpose" — required before a real shadow run counts as promotion-quality evidence, though the first F-051 wiring run may still happen before this lands | `review/DEVIATIONS.md` D-031 |
| HANDOVER.md / README.md / status.md documentation pass (this session, 2026-08-26) | Brought current for a new developer joining the project — HANDOVER.md rewritten against the actual current state (M1/M2 PASSED, F-047/048/050/052 shipped, reconciliation v0, F-051/053/054/CI/contract-review open); README.md's implemented/not-implemented table and layout section updated; a tracker bookkeeping bug (duplicate F-052 row) found and fixed while doing this pass | `HANDOVER.md`, `README.md`, `status.md` |
