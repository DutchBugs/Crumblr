...
## Unreviewed work — `feedback.1.20.md` now triggered

`feedback.1.7.md` through `feedback.1.19.md` have all been processed
(F-026…F-055, O-006, above). **M1 remains PASSED, M2 remains PASSED**
(`status.md` §16 Promotion history); Dashboard v0's visual scope stays
**frozen**. Review 1.19 §17's preferred trigger for the next regular review
is: F-054 hardened + F-055 pinned baseline + F-051 real Pepperstone
validation + a real shadow decision + CI and/or the domain-contract
package, arriving together — though the review also explicitly welcomes an
integration defect exposed by the real F-051 run as its own trigger,
without waiting for the whole bundle. F-054 and F-055 are now both fixed
(see below); F-051, CI and the contract supply remain, all three genuinely
blocked on something this session cannot produce (an MT5 host, `gh`/Actions
access, and a human reviewer respectively).

Pruned per the standing instruction (review 1.13 §13) — everything from
reviews 1.7 through 1.18 has now been seen by a reviewer (1.18 and/or 1.19
...
|---|---|---|
| CI | Still not confirmed running — unchanged since review 1.10; reviews 1.17-1.19 all say this is "no longer an engineering blocker, it is an evidence retrieval task" and ask for commit SHA, Linux job, Windows job, PostgreSQL tests, gitleaks and overall status to be recorded. This session still has no `gh` CLI/Actions access — needs a human to check and record | `.github/workflows/ci.yml` |
| **Domain-contract package for reviewer approval (assembled 2026-08-25)** | Covers all twelve contracts review 1.14 §13 named. Reviews 1.17-1.19 all state the reviewer has **not** been given the actual file yet and M0 contract review is therefore not approved — supplying it unchanged is a required action, not optional | `review/domain_contracts.md` |
| **F-051 — real-terminal verification of F-047/F-048/reconciliation (reconfirmed by review 1.19 §8, an 18-step checklist now also covering F-053/F-054/F-055/D-031 together)** | Not yet done — no MT5 host available in any session so far. Every capability shipped since M1 remains fake-terminal-tested only | — |
| **F-053 — instrument-spec reconciliation (shipped 2026-08-26; comparison target hardened same day, see F-055)** | `reconcile()` compares the observed spec against an explicitly pinned expectation via `spec_version`; missing/changed/unpinned → `UNKNOWN`/`MISMATCHED` | `src/crumblr/application/reconciliation.py` |
| **F-054 — durable live-decision idempotence (shipped 2026-08-26; fail-closed recovery hardened same day after review 1.19 §5 reopened it)** | `LiveDecisionOrchestrator` restores/re-persists decision-window state via `DecisionWindowStore`; an unreadable record now trips the kill switch rather than reading as empty | `src/crumblr/application/decision_window.py`, `src/crumblr/persistence/decision_window.py` |
| **F-055 — instrument-spec baseline must be authorized, not first-observed (new, review 1.19 §4, built and closed same day)** | `config.MarketConfig.expected_spec_version` — a human-approved, git-reviewable pin, `None` (→ `UNKNOWN`) until a real F-051 observation is reviewed and accepted | `src/crumblr/config.py`, `src/crumblr/application/reconciliation.py` |
| **D-031 — feature-value persistence (shipped 2026-08-26)** | `FeatureSnapshotStore` durably records the full `FeatureEvidence` payload (not only its hash) for every evaluated window, wired into both the replay and live paths via `RunRecorder.record_features` | `src/crumblr/persistence/features.py`, `src/crumblr/application/recording.py` |
| **F-033, seventh reopen/close (2026-08-26, review 1.19 §2, non-blocking)** | Two summary-sync drifts in `status.md` corrected (§1's maturity table vs. §3's current status; the overall-health test count vs. the checklist's) | `status.md` §1 |
