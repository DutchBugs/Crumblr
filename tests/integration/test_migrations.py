...
def empty_database(database_available: bool) -> Iterator[Engine]:
    ...
    engine = create_db_engine(DEFAULT_TEST_URL)
    _wipe(engine)
    try:
        yield engine
    finally:
...
class TestTheBaselineBuildsWhatTheCodeExpects:
    def test_migrating_an_empty_database_creates_every_table(self, empty_database: Engine) -> None:
        upgrade_to_head(DEFAULT_TEST_URL)
    ...
    def test_the_baseline_can_be_unwound(self, empty_database: Engine) -> None:
        ...
        upgrade_to_head(DEFAULT_TEST_URL)
        downgrade_to_base(DEFAULT_TEST_URL)
...
class TestABackupCanBeRestored:
    ...
    def test_a_dump_restores_into_a_database_the_application_can_read(
        ...
        dump = subprocess.run(
            [*dump_command, "-U", "crumblr", "-d", "crumblr", "--clean", "--if-exists"],
            capture_output=True,
            text=True,
            check=False,
        )
        ...
        restore = subprocess.run(
            [*restore_command, "-U", "crumblr", "-d", "crumblr", "-v", "ON_ERROR_STOP=1"],
            input=dump.stdout,
            capture_output=True,
            text=True,
            check=False,
        ...
        restored = create_db_engine(DEFAULT_TEST_URL)
        try:
